...
The leaderboard SHALL display each entry's score as a star glyph string (using ★, ½, and ☆ characters) rather than a raw numeric value. The star representation SHALL be derived from a 0–100 percentage scale using the following bracket mapping:
...
The percentage SHALL be computed as `sort_score / 5 × 100`, where `sort_score` is the average rating on the 0.5–5.0 scale.

#### Scenario: Score renders as full stars
- **WHEN** a dish has a `sort_score` that maps to 96–100%
- **THEN** the leaderboard displays ★★★★★ for that entry

#### Scenario: Score renders as half stars
- **WHEN** a dish has a `sort_score` that maps to 86–95%
- **THEN** the leaderboard displays ★★★★½ for that entry

#### Scenario: Score renders empty marker for very low scores
- **WHEN** a dish has a `sort_score` that maps to 0–4%
- **THEN** the leaderboard displays – for that entry

### Requirement: Star rating shows percentage on hover
The star rating display SHALL include a tooltip (via the HTML `title` attribute) showing the exact percentage value (e.g. `"87%"`) when the user hovers over the score.

The percentage SHALL be rounded to the nearest whole number.

#### Scenario: Hover reveals percentage
