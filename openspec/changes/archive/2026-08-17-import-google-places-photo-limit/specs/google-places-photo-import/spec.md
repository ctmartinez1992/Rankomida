...
When creating a new venue the import SHALL request photo metadata from Google Places, download the first available photo, and persist it to `Venue.photo`, `Venue.photo_credit`, and `Venue.photo_source_url`, subject to the per-run `--max-photos` limit. If the venue already has a photo, the import SHALL skip the fetch entirely.
...
- **AND** the per-run photo limit has not been reached
